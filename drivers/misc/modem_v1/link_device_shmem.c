...
/* not in use */
#define IOCTL_MODEM_GIMME_SHIT _IO('o', 0x49)
		       ...
		       unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
	case IOCTL_MODEM_GIMME_SHIT:
	{
		struct shdmem_info mem_info;
		void __user *dst;
		unsigned long size;
		mif_err("%s: IOCTL_MODEM_GIMME_SHIT", ld->name);
		mem_info.base = shm_get_phys_base();
		mem_info.size = shm_get_phys_size();
		mem_info.v_ipc = (unsigned long) (void *) shm_get_ipc_region();
		mem_info.v_ipc_size = (unsigned long) (void *) ((unsigned long) shm_get_ipc_rgn_size());
		mem_info.v_boot = (unsigned long) (void *) shm_get_boot_region();
		mem_info.v_boot_size = (unsigned long) (void *) ((unsigned long) shm_get_boot_size());
		mem_info.mcu_addr = (unsigned long) (void *) 0;
		mem_info.sysram_addr = (unsigned long) (void *) 0;
		dst = (void __user *)arg;
